...
$res=$db->query($sql);
		...
		<button  type="submit" name="addCart" class="btn btn-primary"><i class="fa fa-shopping-cart"></i>Add to Cart</button>
		 ...
		 <input type="button" class="btn btn-primary" id="submit" name="submit" value="Submit"></input>
		<input type="hidden" id="getid" value="'.$_GET['id'].'"></input>
		<input type="hidden" id="getcc" value="'.$_GET['cc'].'"></input>
...
$sql="select * from Gendral_sku where gendral_sku.ID!='{$_GET["id"]}' AND gendral_sku.PRODUCT_NAME='{$_GET["cc"]}'";
$res=$db->query($sql);
			if($res->num_rows>0)
			{
				while($row=$res->fetch_assoc())
	 ...
	 <a  class="btn btn-light" href="product_view.php?id='. $row['ID'] . '&cc='. $row['PRODUCT_NAME'] .'" target="_black"">
      '. $row['VARIATION_TITLE'] .'
